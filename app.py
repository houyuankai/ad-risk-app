...
if app_mode == "🏠 系統首頁":
    ...
    with col1:
        st.info("👋 **歡迎使用 v6.3 阿茲海默雙軌分析系統！**")
        st.markdown("""
        **系統關鍵字導覽：**
        - **🔍 網站操作**：點擊左上角「>>」展開側邊欄選單切換功能。
        - **🤖 功能介紹**：包含 AI 問答、生活習慣分析、MRI醫療分析。
        - **📄 報告生成**：提供一鍵下載 PDF 參考報告。
        
        **五大核心功能：**
        1. **🤖 AI 諮詢**：提供疾病資訊、諮詢與衛教問答。
...
# --- PAGE 2: AI Chatbot ---
    ...
    if prompt := st.chat_input("請輸入您的問題..."):
        ...
        if any(x in q for x in ["操作", "怎麼用", "功能", "教學"]):
            ...
            reply = "🤡 抱歉，我的資料庫目前僅涵蓋「疾病介紹、飲食、運動、睡眠、就醫、費用、網站操作」等主題。您可以試著問：「這個網站怎麼用？」或「要去哪裡看醫生？」"
...
# --- PAGE 4: 臨床落點 (新增ApoE4模擬提示) ---
    ...
    with c1:
        ...
        # [修改] 標註模擬情境
        c_apoe = st.selectbox("ApoE4 基因型 (模擬情境)", ["Negative", "Positive (e3/e4)", "High Risk (e4/e4)"], 
                              help="本選項僅用於模擬基因風險對預測機率的影響，不代表真實基因檢測結果。")
